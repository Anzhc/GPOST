...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.model_combo.addItem("Gemini 2.5 Flash (preview)", ("gemini", "gemini-2.5-flash-preview-04-17"))
        self.model_combo.addItem("Gemini 2.5 Pro (preview)", ("gemini", "gemini-2.5-pro-preview-03-25"))
        self.model_combo.addItem("Gemini 2.0 Flash", ("gemini", "gemini-2.0-flash"))
        self.model_combo.addItem("Gemini 2.0 Flash Lite", ("gemini", "gemini-2.0-flash-lite"))
    ...
    def on_run_inference(self):
        ...
        debug_log(f"inference_parse_ms={int((t_parse_end - t_parse_start) * 1000)} detections={len(self.last_detections)}")
        enabled_classes = self.get_enabled_classes()
        if enabled_classes is not None:
            before = len(self.last_detections)
